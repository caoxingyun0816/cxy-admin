...
public class HttpClientUtil {
	...
	public static String httpClientPutWithBody(String url,String jsonStr){
		PutMethod putMethod = new PutMethod(url);
		//将表单的值放入postMethod中
		try {
			RequestEntity stringRequestEntity = new StringRequestEntity(jsonStr,"application/json","UTF-8");
			putMethod.setRequestEntity(stringRequestEntity);
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}

		return getResponseStr(putMethod);

	}
	public static String httpClientPostWithBody(String url,String jsonStr){
		PostMethod postMethod  = new PostMethod(url);
		try {
			RequestEntity stringRequestEntity = new StringRequestEntity(jsonStr,"application/json","UTF-8");
	...
	public static String httpClientDeleteWithBody(String url,String jsonStr){
		DeleteMethodWithBody deleteMethod  = new DeleteMethodWithBody(url);
		try {
			RequestEntity stringRequestEntity = new StringRequestEntity(jsonStr,"application/json","UTF-8");
			deleteMethod.setRequestEntity(stringRequestEntity);
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		return getResponseStr(deleteMethod);
	 ...
	 * @return
	 */
	public static String httpClientPost(String url, NameValuePair[] values, String xml,String charSet){
		...
		method.addRequestHeader("Content-Type", "text/xml;charset="+charSet);
		RequestEntity reis = null;
		InputStream input = null;
		InputStream is = null;
		BufferedReader dis = null;
		try {
			input = new ByteArrayInputStream(xml.getBytes(charSet));
...
//			e.printStackTrace();
     ...
     * String rst = HttpClientUtil.httpClientPost(url, requestHeaderMap,HttpClientUtil.praseParameterMap(requestParamMap), reqJson);
     */
    public static String httpPostStr(String url, Map<String,String> headerMap, NameValuePair[] values, String xml){
        StringBuilder sb = new StringBuilder();
        log.debug(" http url :" + url);
        for (NameValuePair nvp : values) {
            log.debug(" http param :" + nvp.getName() + " = " + nvp.getValue());
        ...
        if(headerMap != null){
            ...
            for(String header : headerMap.keySet()){
                method.addRequestHeader(header,headerMap.get(header));
            }
        }else{
            method.addRequestHeader("Content-Type", "text/xml;charset=UTF-8");
        }
