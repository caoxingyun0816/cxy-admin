...
public class DES {
	...
	private static final int[][][] S_Box = {
			...
			{ 13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7 },
					...
					{ 2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11 } } // S_Box
																				// [
																				// 8
																				// ]
	};
...
/**
    ...
    public static String hex2bin(String hex) throws UnsupportedEncodingException {
        String digital = "0A1B2C3D4E5F6789";
	...
	public static String strToDes(String data, String key) {
		String strResult = "";
		byte[] byteResult;
		byte[] bytekey = null;
		byte[] bytedata = null;
		try {
			bytekey = key.getBytes(ENCODE_ISO8859_1);
			bytedata = data.getBytes(ENCODE_ISO8859_1);
		} catch (UnsupportedEncodingException e) {
			log.error("errorMsg:"+e.getMessage());
		}
		
		DES des = new DES();
		byteResult = des.DesEncrypt(bytekey, bytedata, 1);
		try {
			strResult = new String(byteResult, ENCODE_ISO8859_1);
		} catch (UnsupportedEncodingException e) {
			log.error("errorMsg:"+e.getMessage());
		}
		return strResult;
	...
	public static String desToStr(String data, String key) {
		String strResult = "";
		byte[] desResult;
		byte[] bytekey = null;
		byte[] bytedata = null;
		...
		DES des = new DES();
		desResult = des.DesEncrypt(bytekey, bytedata, 0);
		
		byte[] byteResult= new byte[bytedata.length - desResult[desResult.length - 9]];
		for (int i = 0; i < byteResult.length; i++) {
			byteResult[i] = desResult[i];
		}
...
//		 String content = "1.0.8bcdef&/g=1.0ds原啊打发";//"nodeUrl=/";//
//	     System.out.println(bin2hex(content));
//	     System.out.println(content);
//	     System.out.println(hex2bin(bin2hex(content)));
	        
//		String key = "abccAADF45213137";
//		String data = "abc&%^%$mm>231258900<<def&/g=1.0d!@#$%^&*&&&&s原啊打发";
//		System.out.println(data);
//		
//		data = URLEncoder.encode(data, DES.ENCODE_UTF_8);
//		String strToDes = DES.strToDes(data, key);
//		System.out.println("strToDes:"+strToDes);
//		
//		String desToStr = DES.desToStr(strToDes, key);
//		desToStr = URLDecoder.decode(desToStr, DES.ENCODE_UTF_8);
//		System.out.println(desToStr);
//	}
}
*/
